...
- **前端两条并行管线**：TRX 走 `RxLineAssembler`→viewportManager→TerminalView（不动）；TTY 走 `ttyService`（按端口持有 xterm 实例引用，实例本身由 `TtyView` 创建并拥有）→`TtyView`。两者经每端口 `mode` 在**三个薄接缝**处分流。
...
- **切换副作用**：清 TerminalStore + `releaseViewportManager`（销毁 TRX 缓冲，切回 TRX 时 TerminalView 挂载重建）+ `getRxPipeline().flushAndReset` + `ttyService.clear`——避免旧模式 buffered 数据混入新模式首屏。
...
serial:data（TTY 端口，useSerialReceive 分流）
  → 流式 UTF-8 解码（每端口一个解码器，经 `src/utils/lineText.ts` 的 `createDecoder` 工厂构建——全仓唯一解码器工厂，`ignoreBOM:false`；解码器持残字节故 per-port 独占，decode 带 `{stream:true}` 缓冲跨事件多字节字符；仅 UTF-8，TRX 的多编码切换不适用 TTY）
  → 每端口队列（上限 `MAX_TTY_QUEUE` 10000 条，丢最旧）
...
pty 行规程（ICRNL）把 `\r` 转成 `\n`，`\r\n` 会变成两个换行——bash 执行完命令还多收到一个空行。`SerialManager::send_data` 经 `PortKind::of` 分派到 `serial/ports_tty.rs::send`，先调 `serial/codec.rs` 的 `normalize_tty_line_ending`：`\r\n` → `\r`（真实终端 Enter，ICRNL 转回单个 `\n`），`\r`/`\n`/`None` 原样保留；归一后的字节再经 `build_tx_bytes` 生成，即 TX 日志记录的**实际写入 pty** 字节。覆盖快捷发送/循环/触发自动回复/弹出窗全部**文本** TX 路径（均经 `send_data`）；文件发送走 `ports_tty.rs::write_raw` 原样写字节，不做归一也不附加行结束符。
...
| 存储 | 模式在 `Port`（useAppStore）；xterm 实例由 `TtyView` 拥有、`ttyService` 仅持引用 | 低 |
...
- **模块划分**：`commands/tty_sim.rs`（3 个命令 `enable_gitbash_sim` / `disable_gitbash_sim` / `resize_gitbash_sim`）· `serial/ports_tty.rs`（GIT: 路由与端口句柄操作 `open` / `close` / `send` / `write_raw` / `resize`）· `serial/tty_sim.rs`（portable-pty/ConPTY 本体：`spawn_bash`、`find_bash`、读线程）· `serial/mod.rs`（类别分派与门控：`PortKind::of(port_id)` 是端口类别分派的唯一入口，`open_port` 在分派前调 `check_virtual_enabled(kind, simulate, gitbash_sim)`；全仓没有 `port_id.starts_with("GIT:")` 式字符串分派）。
...
- **双层门控**：
  1. **命令入口门控**——`commands/system_cmds.rs` 的 `dev_only(capability)` 是后端调试能力门控的**唯一实现**（`is_debug_build()` 是发布通道专属能力的短路条件，如自动更新）：`commands/tty_sim.rs` 三个命令的每份命令体（单一实现）入口都调 `dev_only("Git Bash sim terminal")`，release 构建直接拒绝，不再写 `cfg(not(debug_assertions))` 双分支/双实现。前端同层由 `src/utils/devMode.ts` 的 `DEV_FEATURES_ENABLED`（= `import.meta.env.DEV`，与 `useSimulation` 共用）隐藏 UI，`useGitBashSim` 内再兜底 no-op。
  2. **端口可用性门控**——即使命令被直接 invoke，`SerialManager::open_port` 也按 `PortKind::of(port_id)` 分派并先过 `check_virtual_enabled`（GIT: 需要 `gitbash_sim` 开关），未启用时返回错误且**不 spawn 任何进程**。
  该模块在 release 仍参与编译（自身不 gate），只是没有可用入口。

## 已知边界（文档化而非缺陷）

- RX 解码仅 UTF-8（TRX 的 GBK 等多编码切换不适用于 TTY 字节流）。
- 残余销毁场景：模式切换/关闭标签/跨 Pane 拖拽仍销毁 xterm 实例（open() 一次性）。
- 日志：TTY 端口 RX 若沿用 LogLineAssembler（CR 当换行）会剁碎终端序列——当前按既有日志管线落盘，原始字节保真回放尚未实现。
