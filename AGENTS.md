...
| 插件能力补强（issue #17 能力） | `utils/pluginHostApi.ts`（`fs.openDialog`/`ui.panel`）+ `utils/pluginBytesObserver.ts`（`rx.onBytes`）+ `utils/pluginPanelRegistry.ts` + `shared/PluginPanel.tsx` | `fs.openDialog`：用户经系统对话框选任意文件（GBK 兼容，权限 `fs:open`，后端读 base64 原始字节）；`ui.panel.append/clear/export`：插件专属输出面板（512KB 上限截断丢最旧）；`rx.onBytes`：原始字节旁路（serial:data 事件层、不分 mode、权限 `rx:bytes`），供 HTTP 推送第三方 |
...
| `PluginPanel` | `src/components/shared/PluginPanel.tsx` | component | 插件输出面板（issue #17 能力补强）：渲染 worker 经 `plugin.api.ui.panel.append/clear` 写入的聚合输出；订阅 `pluginPanelRegistry` 快照（`usePluginPanel` 组件本地订阅 helper，与 `usePluginUi` 同级、不单独计数）；512KB 上限截断丢最旧；空面板不渲染 | 
...
| `usePluginHost` / `usePluginList` | 插件系统（issue #17）：usePluginHost 主窗单例装配（worker 生命周期 syncWithConfig + list_plugins 写回 store.config.pluginConfigs + config 订阅驱动的启停/rx 行+字节双装配 + 面板清理 + 崩溃回调）；usePluginList 设置页列表 + 命令（install/uninstall/enable/permissions，命令返回状态数组写回 store）。能力补强：`fs.openDialog`（用户对话框选任意文件，GBK 兼容，权限 `fs:open`）+ `ui.panel`（512KB 上限输出面板）+ `rx.onBytes`（原始字节旁路，`rx:bytes` 权限） | usePluginHost: App.tsx (once)；usePluginList: PluginSettings |
...
| `file.rs` | write_text_file, read_text_file（配置导入导出）, read_file_bytes（base64 原始字节——插件 fs.openDialog 用，GBK 兼容）, read_image_data_url（背景图 data URL，issue #13） |
