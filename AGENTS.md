...
│   ├── i18n.ts                   # i18next + react-i18next, ~556 keys × zh-CN/en-US
...
- `src/i18n.ts` 已就位 — i18next + react-i18next，扁平 dotted key（`keySeparator: false`），556 keys × zh-CN/en-US

  > 2026-08-15 issue #12 自动更新新增 `update.*` 28 键，复审删除 8 个无消费者键（newVersion/publishDate/downloadFailed/upToDate 双语）→ 净 550；二轮新增 viewRelease/lastCheckLabel/lastCheckNever（双语 6 条）→ 556。
