...
- 🔄 **自动更新** — 通道运行时用户选择：设置项「不检查 / 定期到正式版 / 定期到 preview」（7 天周期 + 会话内周期重评估，常驻挂机也覆盖）；发现更新弹窗三动作（立即更新带进度 / 7 天后提醒 / 永不提醒同步设置）+ 发布页链接；About 手动检查可选正式版/preview；stable 直连 GitHub 最新正式版、preview 经 GitHub API 解析最新 preview tag（唯一 tag，互不污染；**preview 检查同时取 preview/stable semver 大者**——收尾发布 stable 后自动晋升、热修不缺）。macOS 暂不支持自动更新（未签名/公证，产物可手动下载安装）
