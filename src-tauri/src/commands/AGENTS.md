...
| `storage.rs` | settings entities CRUD (command sets / highlight sets / protocol templates / trigger rules / port presets / tool configs). Synchronous ConfigManager operations — lock `config_manager`, mutate the entity Vec in `AppConfig`, save config.json. NO SQLite/async/transactions. |
