...
| `SendSection.tsx` | ~360 | compact header (`.op-send-header`: card title on the left; command-set `<select>` + loop icon-toggle + edit icon on the right, one line) above the quick-send pill row + send textarea + HEX toggle (bidirectional content conversion via `textToHexPreview`/`hexToTextPreview`) + HEX input sanitization (`sanitizeHexInput`) + line terminator selector + file-send button; the loop toggle (`handleToggleLoop`) 按**当前聚焦端口**启停该端口的独立循环（`setCyclicLoop(activeTabId, bool)`，issue #12）并脉动显示; repeat count is NOT here — it lives per command set (`SendCommandSet.repeatCount`); Enter semantics governed by `config.sendOnEnter` (plain Enter sends by default; Shift/Ctrl+Enter always insert newline) |
| `ParamsSection.tsx` | 236 | baudRate / dataBits / parity / stopBits / handshake / DTR / RTS; preset apply dropdown only (preset management moved to `ConfigModal/pages/GeneralSettings.tsx`); **TRX/TTY 模式分段控件**（`.segmented`，issue #11）——`handleModeChange` 写 `setPortMode` + 清 TerminalStore + `getRxPipeline().flushAndReset` + `ttyService.clear`（避免旧模式 buffered 数据混入新模式首屏） |
| `hooks/useCyclicSend.ts` | ~200 | 每端口独立循环引擎（issue #12）：store 运行标志 `useOperationStore.cyclicLoops[portId]` → 每个端口一个 runtime（目标端口启动时绑定、聚焦无关、多端口并行），每 tick 经 `useRuleStore.getState()` 重读活动命令集；round boundary detected by `currentCmdIdx === length-1` (index resets to 0 each round so per-command `delay` keeps applying and `completedRounds` counts rounds, not commands); repeat limit from the active set's `repeatCount` (0 = follow `isLoop`); 目标端口未连接 → 跳过 tick 500ms 重试（不中断）；error retry fixed at 500ms；`visibilitychange` 恢复可见补发到期 tick |
...
- **`usePanelCyclicSend`（弹窗文本循环）ref 含 `nextFireAt`**：与主窗 `useCyclicSend` 同样做可见性补发——窗口被置顶同款遮挡恢复后节奏立即拉回配置值。
