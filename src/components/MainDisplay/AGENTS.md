...
| `Pane.tsx` | 308 | leaf shell: TabBar + terminal container; **TTY 标签常驻挂载**（当前 Pane 内所有 `mode==='tty'` 端口各渲染一个 `TtyView`，`key={tab.id}`，非活动标签传 `hidden` → `.tty-view-hidden` display:none，会话跨标签切换保留），`displayPort?.mode !== 'tty'` 才渲染 `TerminalView`（TRX 缓冲在 store，按需挂载）；blocks pop-out for TTY ports (`tty.popoutUnsupported` toast) |
| `TtyView.tsx` | 208 | xterm.js TTY host (issue #11): creates `Terminal` + FitAddon per port (fonts/theme from config + CSS vars), `term.open` + ResizeObserver+rAF-debounced fit, onData→`ttyService.send`, onResize→`ttyService.resize`; `hidden` prop = 非活动标签（display:none，恢复可见显式 re-fit，**会话跨标签保留**）; 字体/字号经 `term.options` 活更新不重建; Ctrl+滚轮缩放（镜像 TerminalView）; **背景图毛玻璃（issue #13）**：`backgroundImageEnabled` 切换时按 config + `data-theme` 构造 rgba 活更新 `term.options.theme.background`（xterm 创建时快照，不读 CSS 变量避免与 ThemeProvider 父 effect 时序竞态）; owns the Terminal instance (dispose on unmount, `ttyService.detach` keeps it) |
...
| `TerminalView.tsx` | 463 | 方案B 直接 DOM 渲染薄壳（issue #14）：hosts scroll container + `TerminalViewportManager` 渲染器 attach/detach（`useLayoutEffect`，issue #15 防跨 Pane 位移 insertBefore 崩溃）；同步显示配置（字体/格式/编码/时间戳/高亮规则）到 renderer；拥有 UI 局部交互态（过滤/搜索/选区/右键菜单/手势 scroll-lock/快速跳转按钮）；协议模板选择条。行渲染由 `TerminalRenderer` 直接操作 DOM，React 不参与行级渲染 |
| `TerminalRenderer.ts` | 704 | direct-DOM 渲染引擎（`utils/terminal/`）：节点池复用 + 稳定 seq + 固定行高 + 同帧钉底 + `insertRowInOrder` 保 DOM 序 == 视觉序；search hit-rows 经 `markSearchMatchesInHtml` 叠加字符级 `<mark>`（`searchQuery`/`searchCaseSensitive` 来自 view state） |
...
- Closing a tab without going through `cleanupClosedTab` in `Pane.tsx` — bypass leaks the RX pipeline state / viewport manager / popout window. (issue #11：关闭标签页**不**关闭串口——`closePort` 由用户显式触发，标签关闭只销毁前端显示目标。)
