...
| Store | File:line | Job |
|-------|-----------|-----|
| `useAppStore` | `useAppStore.ts:266` | tabs / ports / `paneTree` / config / groups — `setGroups` bulk-loads persisted groups at startup; group changes auto-save (500ms debounce in `useAppInit`) |
